...
module.exports = function (router) {
  ...
  // DISABLED FOR DEVELOPMENT - Email credentials are invalid
  ...
  // Mock sendMail function that doesn't send emails but prevents errors
  var mockSendMail = function (email, callback) {
    if (callback) callback(null, { response: "Email mocked (not sent)" });
  };
  ...
  router.post("/users", function (req, res) {
    ...
    if (
      ...
      // FOR DEVELOPMENT: Auto-activate accounts instead of requiring email confirmation
      user.active = true;
      ...
      user.save(function (err) {
        if (err) {
          ...
          if (err.errors) {
            if (err.errors.name && err.errors.name.message) {
            ...
            } else if (err.errors.username && err.errors.username.message) {
              res.json({
                success: false,
              ...
              res.json({ success: false, message: JSON.stringify(err) }); // Display any other errors with validation
            }
            ...
            if (err.errmsg && err.errmsg.includes("username")) {
              ...
              res.json({ success: false, message: "Duplicate entry error" });
            }
          } else {
            res.json({ success: false, message: JSON.stringify(err) }); // Display any other error
          }
        } else {
          // Create e-mail object to send to user (mocked)
          var email = {
            ...
            text:
              "Hello " +
              user.name +
              ", thank you for registering at localhost.com. Your account is now active!",
            html:
              "Hello<strong> " +
              user.name +
              "</strong>,<br><br>Thank you for registering at localhost.com. Your account is now active!",
          };
          // Function to send e-mail to the user (using mock)
          mockSendMail(email, function (err, info) {
            if (err) {
              console.log("Mock email (not sent): " + err); // If error with sending e-mail, log to console/terminal
            } else {
              console.log("Mock email sent to: " + user.email); // Log success message to console if sent
            }
          });
          res.json({
            success: true,
            message: "Account registered! Your account is ready to use.",
      ...
      .exec(function (err, user) {
        if (err) {
          ...
          var email = {
            ...
            text:
              "The following error has been reported in the MEAN Stack Application: " +
              err,
            html:
              "The following error has been reported in the MEAN Stack Application:<br><br>" +
              err,
          };
          ...
          res.json({
            success: false,
            message:
              "Something went wrong. This error has been logged and will be addressed by our staff. We apologize for this inconvenience!",
          });
        } else {
          if (user) {
            res.json({
            ...
            if (!req.body.password) {
              ...
              if (!validPassword) {
                ...
                res.json({
                  success: false,
                  message:
                    "Account is not yet activated. Please check your e-mail for activation link.",
                  expired: true,
            ...
            if (err) {
              ...
              var email = {
                ...
                text:
                  "Hello " +
                  user.name +
                  ", You recently requested a new account activation link. Please click on the following link to complete your activation: https://immense-dusk-71112.herokuapp.com/activate/" +
                  user.temporarytoken,
                html:
                  "Hello<strong> " +
                  user.name +
                  '</strong>,<br><br>You recently requested a new account activation link. Please click on the link below to complete your activation:<br><br><a href="http://www.herokutestapp3z24.com/activate/' +
                  user.temporarytoken +
                  '">http://www.herokutestapp3z24.com/activate/</a>',
              };
          ...
          if (!user) {
            ...
            var email = {
              ...
              text:
                "Hello " +
                user.name +
                ", You recently requested your username. Please save it in your files: " +
                user.username,
              html:
                "Hello<strong> " +
                user.name +
                "</strong>,<br><br>You recently requested your username. Please save it in your files: " +
                user.username,
            };
              ...
              if (err) {
                ...
                var email = {
                  ...
                  text:
                    "Hello " +
                    user.name +
                    ', You recently request a password reset link. Please click on the link below to reset your password:<br><br><a href="http://www.herokutestapp3z24.com/reset/' +
                    user.resettoken,
                  html:
                    "Hello<strong> " +
                    user.name +
                    '</strong>,<br><br>You recently request a password reset link. Please click on the link below to reset your password:<br><br><a href="http://www.herokutestapp3z24.com/reset/' +
                    user.resettoken +
                    '">http://www.herokutestapp3z24.com/reset/</a>',
                };
              ...
              if (err) {
                ...
                var email = {
                  ...
                  html:
                    "Hello<strong> " +
                    user.name +
                    "</strong>,<br><br>This e-mail is to notify you that your password was recently reset at localhost.com",
                };
                // Function to send e-mail to the user
                mockSendMail(email, function (err, info) {
  ...
  router.use(function (req, res, next) {
    var token =
      req.body.token || req.body.query || req.headers["x-access-token"]; // Check for token in body, URL, or headers
          ...
          function (err, mainUser) {
            if (err) {
              ...
              if (!mainUser) {
                ...
                if (
                  mainUser.permission === "admin" ||
                  mainUser.permission === "moderator"
                ) {
                      ...
                      if (user.permission === "admin") {
                        ...
                        if (mainUser.permission !== "admin") {
                          res.json({
                            success: false,
                            message:
                              "Insufficient Permissions. You must be an admin to downgrade an admin.",
                          }); // Return error
                        } else {
                      ...
                      if (user.permission === "admin") {
                        ...
                        if (mainUser.permission !== "admin") {
                          res.json({
                            success: false,
                            message:
                              "Insufficient Permissions. You must be an admin to downgrade another admin",
                          }); // Return error
                        } else {
                    ...
                    if (newPermission === "admin") {
                      ...
                      if (mainUser.permission === "admin") {
                        ...
                        res.json({
                          success: false,
                          message:
                            "Insufficient Permissions. You must be an admin to upgrade someone to the admin level",
                        }); // Return error
                      }
                    }
                  }
